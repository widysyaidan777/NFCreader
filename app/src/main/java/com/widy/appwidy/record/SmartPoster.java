...
public class SmartPoster implements ParserNdefRecord{
    ...
    public static SmartPoster parse(NdefRecord[] recordsRaw){
        try {
            Iterable<ParserNdefRecord> records = NdefMessageParser.getRecords(recordsRaw);
            UriRecord uri = Iterables.getOnlyElement(Iterables.filter(records, UriRecord.class));
            TextRecord title = getFirstIfExist(records, TextRecord.class);
    ...
    private static <T> T getFirstIfExist(Iterable<?> elements, Class<T> type) {
